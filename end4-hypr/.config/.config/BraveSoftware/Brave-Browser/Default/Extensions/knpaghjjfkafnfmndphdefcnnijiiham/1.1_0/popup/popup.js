...
function saveSettings() {
    const settings = {
        text_type: document.querySelector('select[name="text_type"]').value,
        size_type: document.querySelector('select[name="size_type"]').value,
        count: document.querySelector('input[name="count"]').value,
        add_p_tag: document.querySelector('input[name="add_p_tag"]').checked
    };
    
    chrome.storage.local.set({ 'lorem_ipsum_settings': settings }, function() {
        console.log('Настройки сохранены:', settings);
    });
}

// Функция для сохранения в историю Last Used
function saveToHistory() {
    const textType = document.querySelector('select[name="text_type"]').value;
    const sizeTypeSelect = document.querySelector('select[name="size_type"]');
    const sizeType = sizeTypeSelect.options[sizeTypeSelect.selectedIndex].text.toLowerCase();
    const count = document.querySelector('input[name="count"]').value;
    const addPTag = document.querySelector('input[name="add_p_tag"]').checked;
...
function loadSettings() {
    chrome.storage.local.get(['lorem_ipsum_settings'], function(result) {
        if (result.lorem_ipsum_settings) {
            const settings = result.lorem_ipsum_settings;
            
            // Восстанавливаем значения
            document.querySelector('select[name="text_type"]').value = settings.text_type || '1';
            document.querySelector('select[name="size_type"]').value = settings.size_type || '1';
            document.querySelector('input[name="count"]').value = settings.count || '5';
            document.querySelector('input[name="add_p_tag"]').checked = settings.add_p_tag || false;
...
{
    var p_checkbox = document.querySelector('input[name="add_p_tag"]').checked;

    var text_type = document.querySelector('select[name="text_type"]').value;

    if(text_type == '1')
    {
        var text_data = lorem_ipsum_text;
    }
    else if(text_type == '2')
    {
        var text_data = marketing_text;
    }
    else if(text_type == '3')
    {
        var text_data = scientific_text;
    }
    else if(text_type == '4')
    {
        var text_data = literary_text;
    }
    else if(text_type == '5')
    {
        var text_data = technical_text;
    }
    else if(text_type == '6')
    {
        var text_data = legal_text;
    }
    else if(text_type == '7')
    {
        var text_data = ecommerce_text;
    }
    else if(text_type == '8')
    {
        var text_data = social_media_text;
    }

    var count_elements = document.querySelector('input[name="count"]').value;
    var size_type = document.querySelector('select[name="size_type"]').value;
    
    var result_array = [];
    var shuffledArray = [...text_data];
    ...
    if(size_type == '1')
    {
        ...
        for (let i = 0; i < count_elements; i++) {
            result_array.push(shuffledArray[i % shuffledArray.length]);
        }

        if(p_checkbox)
        {
            result_array = result_array.map(paragraph => `<p>${paragraph}</p>`);
        }
    ...
    {
        // sentences
        var final_array = splitTextIntoSentences(shuffledArray.join(' '));
        
        shuffleArray(final_array);

        for (let i = 0; i < count_elements; i++) {
            result_array.push(final_array[i % final_array.length]);
        }
    ...
    else if(size_type == '3')
    {
        ...
        final_array = mergeSentences(final_array.join(' '));
        ...
        pre_result = result_array.join(" ");
        result = ensurePeriodAtEnd(pre_result);
    ...
    {
        // symbols
        result = generateRandomString(count_elements);
    ...
    generateButton.textContent = "Generate & copy another text";
    changeCopyButtonText();

    copyButton.style.display = 'block';
    
    // Показываем кнопку "Save preset" и сбрасываем её текст
    const savePresetButton = document.querySelector('button[name="save_preset"]');
    savePresetButton.style.display = 'block';
    savePresetButton.textContent = 'Save preset';
    
    // Автоматически копируем текст после генерации
    copyToClipboard();
    
    // Сохраняем настройки
    saveSettings();
    
    // Сохраняем в историю Last Used
    saveToHistory();
    
    // Обновляем блок Last Used, если он открыт
    const lastUsedBlock = document.getElementById('last_used_block');
    if (lastUsedBlock && lastUsedBlock.style.display === 'block') {
        loadLastUsed();
    }

    countGenerated++;
    const rate_block = document.getElementById('full_stars_block');

    if (localStorage.getItem('saved_rate') === '1') {
        var saved_rate = 1;
    } else {
        var saved_rate = 0;
    }

    if(countGenerated == 10 && rate_block.style.display != 'block' && saved_rate != 1)
    {
...
function ensurePeriodAtEnd(text) {
    if (text.endsWith(',')) {
...
function mergeSentences(text) {
    const sentences = text.match(/[^.!?]+[.!?]+[\s]*/g) || [];
    const mergedText = sentences
        .map((sentence, index) => {
...
function changeButtonText() {
    generateButton.textContent = "Generate & copy text";
}
...
function copyToClipboard() {
    const textarea = document.querySelector('textarea[name="result"]');
    textarea.select();
    textarea.setSelectionRange(0, 99999);
    const successful = document.execCommand('copy');

    copyButton.textContent = "Copied!";
    setTimeout(changeCopyButtonText, 2000);
}

const generateButton = document.querySelector('button[name="generate_text"]');
generateButton.addEventListener('click', generate_text);

const copyButton = document.querySelector('button[name="copy_text"]');
copyButton.addEventListener('click', copyToClipboard);

const rateTextBtn = document.getElementById('a_show_rate_block');
rateTextBtn.addEventListener('click', show_rate_block);

// Обработчики для пресетов
const savePresetButton = document.querySelector('button[name="save_preset"]');
savePresetButton.addEventListener('click', savePreset);

const managePresetsBtn = document.getElementById('a_show_manage_presets');
managePresetsBtn.addEventListener('click', function() {
    const block = document.getElementById('manage_presets_block');
...
// Обработчик для кнопки Last Used
document.getElementById('a_show_last_used').addEventListener('click', function() {
    const block = document.getElementById('last_used_block');
...
// Обработчик для кнопок пресетов и истории
document.addEventListener('click', function(e) {
    if (e.target.classList.contains('preset-generate')) {
...
document.querySelector('select[name="size_type"]').addEventListener('change', changeButtonText);
document.querySelector('select[name="text_type"]').addEventListener('change', changeButtonText);
document.querySelector('input[name="count"]').addEventListener('change', changeButtonText);

function show_rate_block()
{
    const block = document.getElementById('full_stars_block');
    block.style.display = block.style.display != 'block' ? 'block' : 'none';
}

var countGenerated = 0;

document.querySelectorAll('.save_rate').forEach(link => {
...
function save_rate() {
    localStorage.setItem('saved_rate', '1');
...
function savePreset() {
    ...
    chrome.runtime.sendMessage({
        ...
        if (response && response.success) {
            ...
            const originalText = savePresetButton.textContent;
            savePresetButton.textContent = 'Preset saved';
            
            setTimeout(() => {
                savePresetButton.textContent = originalText;
            }, 3000);
            
            // Обновляем список пресетов в блоке управления
            loadPresets();
        }
    });
}
...
function loadLastUsed() {
    chrome.runtime.sendMessage({ action: "getHistory" }, function(response) {
        if (response && response.history) {
            displayLastUsed(response.history);
        }
    });
}
...
function displayLastUsed(history) {
    const container = document.getElementById('last_used_container');
    container.innerHTML = '';
    
    if (history.length === 0) {
        container.innerHTML = '<div class="no-presets">No history available</div>';
        return;
    }
    
    history.forEach((item, index) => {
        const historyItem = document.createElement('div');
        historyItem.className = 'preset-item';
        historyItem.innerHTML = `
            <span class="preset-name">${item.displayName}</span>
            <button class="preset-generate" data-index="${index}" data-type="history" title="Generate text with this history item">▶</button>
            <button class="preset-delete" data-index="${index}" data-type="history">×</button>
        `;
...
function displayPresets(presets) {
    const container = document.getElementById('presets_container');
    container.innerHTML = '';
    
    if (presets.length === 0) {
        container.innerHTML = '<div class="no-presets">No presets saved yet</div>';
        return;
    }
    ...
    presets.forEach((preset, index) => {
        ...
        presetItem.innerHTML = `
            ...
            <button class="preset-generate" data-index="${index}" data-type="preset" title="Generate text with this preset">▶</button>
            <button class="preset-delete" data-index="${index}" data-type="preset">×</button>
        `;
...
function applyHistoryItem(index) {
    chrome.runtime.sendMessage({
        ...
        if (response && response.history && response.history[index]) {
            const item = response.history[index];
            
            // Заполняем форму настройками из истории
            document.querySelector('select[name="text_type"]').value = item.textTypeId;
            document.querySelector('input[name="count"]').value = item.count;
            document.querySelector('input[name="add_p_tag"]').checked = item.addPTag || false;
            
            // Устанавливаем size_type по тексту
            const sizeTypeSelect = document.querySelector('select[name="size_type"]');
            const sizeTypeText = item.sizeType.toLowerCase();
            for (let i = 0; i < sizeTypeSelect.options.length; i++) {
                if (sizeTypeSelect.options[i].text.toLowerCase() === sizeTypeText) {
                    sizeTypeSelect.selectedIndex = i;
                    break;
                }
            }
            ...
            // Плавно прокручиваем вверх, чтобы показать результат
            setTimeout(() => {
                window.scrollTo({
                    top: 0,
                    behavior: 'smooth'
...
function applyPreset(index) {
    chrome.runtime.sendMessage({
        ...
        if (response && response.presets && response.presets[index]) {
            const preset = response.presets[index];
            
            // Заполняем форму настройками пресета
            document.querySelector('select[name="text_type"]').value = preset.textTypeId;
            document.querySelector('input[name="count"]').value = preset.count;
            document.querySelector('input[name="add_p_tag"]').checked = preset.addPTag || false;
            
            // Устанавливаем size_type по тексту
            const sizeTypeSelect = document.querySelector('select[name="size_type"]');
            const sizeTypeText = preset.sizeType.toLowerCase();
            for (let i = 0; i < sizeTypeSelect.options.length; i++) {
                if (sizeTypeSelect.options[i].text.toLowerCase() === sizeTypeText) {
                    sizeTypeSelect.selectedIndex = i;
                    break;
...
function deleteHistoryItem(index) {
    chrome.runtime.sendMessage({
        ...
        if (response && response.success) {
            loadLastUsed(); // Перезагружаем список
        }
    });
}
...
function deletePreset(index) {
    chrome.runtime.sendMessage({
        ...
        if (response && response.success) {
            loadPresets(); // Перезагружаем список
        }
    });
}

// localStorage.removeItem('closed_update_v_111');

document.addEventListener('DOMContentLoaded', function() {
    // Загружаем сохраненные настройки
    loadSettings();
    
    if (localStorage.getItem('closed_update_v_111') !== 'true') {
    ...
    document.getElementById('close_notification').addEventListener('click', function() {
        ...
        localStorage.setItem('closed_update_v_111', 'true');
    });
});