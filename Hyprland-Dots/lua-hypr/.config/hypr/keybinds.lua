...
hl.bind(mainMod .. " + M", hl.dsp.window.fullscreen({ state = 1 }))
...
hl.bind("CTRL + SHIFT + Print", hl.dsp.exec_cmd("wayfreeze & PID=$!; sleep .1; grim -g \"$(slurp)\" ~/Pictures/$(date +'%Y-%m-%d,%H:%M:%S').png; kill $PID; notify-send 'Saved region'"))
hl.bind("Print", hl.dsp.exec_cmd("wayfreeze & PID=$!; sleep .1; grim -g \"$(slurp)\" - | wl-copy; kill $PID; notify-send 'Copied region'"))
